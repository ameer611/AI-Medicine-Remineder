...
async def extract_text(image_bytes: bytes, filename: str = "prescription.jpg") -> str:
    ...
    endpoint = GEMINI_ENDPOINT_TMPL.format(model=GEMINI_MODEL, api_key=settings.GEMINI_API_KEY)
    payload = {
        "contents": [
            {
                "role": "user",
        ...
        for attempt in range(3):
            try:
                ...
                if exc.response.status_code in (500, 502, 503, 504):
                    logger.warning("Gemini API error %s on attempt %d. Retrying...", exc.response.status_code, attempt + 1)
                    await asyncio.sleep(2 ** attempt)
                    continue
                else:
                    logger.error("Gemini OCR HTTP error: %s", exc)
